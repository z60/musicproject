...
export const CUE_MODIFIERS: readonly string[] = [
...
export const NON_NAME_TOKENS: readonly string[] = [
  ...
  '众人', '大家', '所有人', '旁人', '有人', '此人', '那人', '这人', '对方',
  '老者', '少年', '青年', '中年', '汉子', '男子', '女子', '女孩', '男孩',
  // 说话方式状语（真机实测：`叶海摊摊手，无奈道：` 会把「无奈」当成候选角色）
  '无奈', '疑惑', '好奇', '满意', '沉默', '苦笑', '冷笑', '大笑', '微笑', '狞笑',
  '急忙', '连忙', '赶紧', '匆忙', '认真', '平静', '淡淡', '冷冷', '低声', '轻声',
  '大声', '小声', '不屑', '撇嘴', '点头', '摇头', '皱眉', '叹气', '嘟囔',
  '嘀咕', '沉吟', '思索', '犹豫', '慌张', '紧张', '惊恐', '惊愕', '愤怒', '兴奋',
  '恭维', '恭敬', '客气', '温柔', '严肃', '郑重', '缓缓', '慢慢', '轻轻', '急急',
  // 肢体动作（同样常出现在引导语前：`叶海耸耸肩道：`；叠词形式要单列，否则只会剥到「耸肩」）
  '耸肩', '耸耸肩', '摆手', '摆摆手', '摊手', '摊摊手', '拍手', '拍肩', '咧嘴', '咧咧嘴',
  '咬牙', '咬咬牙', '跺脚', '跺跺脚', '转身', '回头', '眨眼', '眨眨眼', '点头', '点点头',
  '摇头', '摇摇头', '皱眉', '皱皱眉', '叹气', '叹口气', '拱手', '抱拳', '抬手', '抬手一挥',
  // 言语行为动词（`他解释道：` / `叶海补充道：` —— 这些是动词，不可能是人名）
  '继续', '解释', '回答', '补充', '强调', '提醒', '询问', '追问', '商量', '讨论',
  '表示', '认为', '觉得', '赞同', '反对', '抱怨', '感叹', '感慨', '承认', '否认',
  '保证', '承诺', '介绍', '建议', '决定', '命令', '催促', '安慰', '警告', '宣布',
  '回应', '反问', '打断', '插嘴', '接话', '开口', '说话', '出声',
  '报告', '情报', '消息', '情况', '计划', '意见', '想法', '吩咐', '交待', '交代',
  /**
   * 高频副词/名词/泛称（整本书抽取时它们会出现几千次，靠「出现次数」根本筛不掉 ——
   * 真机实测：`叶海` 9342 次、`自己` 2728 次、`时候` 2010 次、`家伙` 1063 次）。
   * 这份表是**经验词表**：理想方案是 NER（docs/06 §5.1 信号 3），没有模型时靠它兜底；
   * 用户还可以通过 `opts.stopwords` 追加自己书里的误抽词。
   */
  '自己', '本人', '时候', '突然', '立刻', '马上', '甚至', '直接', '准备', '位置',
  '随后', '身边', '起来', '下去', '过来', '过去', '出来', '进来', '回来', '下来',
  '家伙', '东西', '事情', '样子', '声音', '方向', '地方', '时间', '世界', '眼睛',
  '脑袋', '心情', '目光', '脸上', '手中', '心里', '身上', '嘴里', '眼前', '周围',
  // 职务/泛称（是「角色」而不是「人名」；用户需要的话可以手动添加）
  '先生', '小姐', '女士', '夫人', '将军', '士兵', '军官', '军阀', '议员', '队长',
  '老板', '医生', '护士', '警察', '记者', '商人', '工人', '农民', '老人', '孩子',
  '男人', '女人', '青年', '朋友', '同伴', '对手', '敌人', '客人', '主人', '人群',
  '同时', '心中', '一边', '一声', '一般', '依旧', '依然', '仍然', '当然', '显然',
  '果然', '终于', '于是', '然后', '结果', '问题', '办法', '机会', '力量', '感觉',
  '意识', '表情', '眼神', '语气', '动作', '反应', '议长', '首领', '人物', '身影',
  '药剂', '眉头', '额头', '嘴角', '手指', '肩膀', '胸口', '伤口', '血脉',
] as const

/**
 * 名字里**几乎不可能出现**的汉字（任何位置）：结构助词、时态助词、语气词。
 *
...
/**
 * 名字里几乎不可能出现的字 —— **只看首尾**（`一` 出现在中间的名字是存在的，如「一鸣」）。
 *
 * 首字命中 → 「一通 / 对船员 / 或者 / 那家伙」这类；尾字命中 → 「模样不 / 微微一」这类。
 */
const NAME_EDGE_REJECT_CHARS = new Set(
  // 功能字/代词（首尾都不该出现）。注意**刻意不含 老/小/大**：
  // 「老张 / 小王 / 大柱」正是要抽出来的称谓，把它们挡掉会让抽取漏掉整类角色。
  '一不没要能会或者与和在被把让给对从向往都就还又便却只也很太更最我你他她它们这那什谁哪多么并且但若如因所以及虽则乃于其此该些个两名每各几是有'
    .split('')
 ...
 *
 * 为什么需要它：`guessNameFromWindow` 的规则是「取引导语前窗口的末尾 2~4 字」——
 * 对 `萧炎沉声道：` 是对的，但 `无奈道：` 也会给出「无奈」、`继续开口说道：` 会给出
 * 「续开口」。**光靠这个窗口规则无法区分「人名」与「状语/动词碎片」**，
 * 所以在抽候选时必须再过一道字型检查（真正的解法是 NER，见 docs/06 §5.1 信号 3；
 * 没有模型时它是 L1 的兜底）。
 *
 * 判据（都是「人名不可能长这样」级别的强特征，宁可少报也不误杀）：
 *   1. 长度 2~4 字（中文姓名 2~3 字，带称谓 4 字）
 *   2. 只含汉字/字母/间隔号
 *   3. 任何位置都不含结构助词、时态助词、语气词（的/地/得/着/了/过/吗/呢…）
 *   4. 首尾不含功能字（一/不/对/或/这/那/我/你…）
 *   5. 不是 AA 式叠词（耸耸肩 / 撇撇嘴 / 刚刚）
 *   6. 不包含言语引导语本身（说道/开口/笑道…）—— 名字里不会出现「说道」
 */
export function looksLikePersonName(token: string): boolean {
  if (!token) return false
  const t = token.trim()
  // 含间隔号的译名（`诺顿·阿兰`）允许到 6 字；纯中文姓名 2~4 字
  const maxLen = t.includes('·') ? 6 : 4
  if (t.length < 2 || t.length > maxLen) return false
  if (!/^[\u4e00-\u9fa5A-Za-z·]+$/.test(t)) return false
  // 间隔号只能在中间（`诺顿·阿兰` ✓；`·阿兰` / `阿兰·` ✗ 是切分残渣）
  if (t.startsWith('·') || t.endsWith('·')) return false
  for (const ch of t) {
    if (NAME_IMPOSSIBLE_CHARS.has(ch)) return false
  }
  const first = t[0]!
  ...
  // 引导语（长度 ≥ 2 的才检查：单字「道/说」在极少数人名里可能出现）
  for (const verb of CUE_VERBS) {
    if (verb.length >= 2 && t.includes(verb)) return false
  }
  for (const verb of INNER_CUE_VERBS) {
    if (verb.length >= 2 && t.includes(verb)) return false
  }
  return true
}
...
export function guessNameFromWindow(window: string): string | null {
  const w = stripCueModifiers(window)
...
/** 反复剥掉窗口结尾的修饰成分（「药老皱眉」→「药老」） */
function stripCueModifiers(window: string): string {
  let w = window
  let changed = true
  while (changed) {
    changed = false
    /**
     * 修饰语剥除一律**从长到短**试。
     * 反例：`舍沙耸耸肩道` 的窗口是「舍沙耸耸肩」，若先剥「耸肩」会剩下「舍沙耸」——
     * 它恰好也能通过字型检查，于是又冒出一个假候选。先剥长的（耸耸肩）才对。
     */
    const candidates = [...CUE_MODIFIERS, ...NON_NAME_TOKENS].filter((x) => x.length >= 2).sort((a, b) => b.length - a.length)
    ...
    /**
     * 修饰语有时被引导语动词**吃掉尾巴**，窗口里只剩前半截：
     * `叶海微微一笑道：` 的窗口是「叶海微微一」（`笑` 被并进了动词），
     * 完整修饰语「微微一笑」匹配不上。所以再试一遍「已知修饰语的前缀」。
     *
     * 只试长度 ≥ 3 的修饰语的、长度 ≥ 2 的前缀 —— 单字修饰语（又/才/便…）
     * 的前缀会把普通名字的末字误剥（`陈又` → 只剩「陈」）。
     */
    for (const mod of CUE_MODIFIERS) {
      if (mod.length < 3) continue
      for (let len = mod.length - 1; len >= 2; len--) {
        const head = mod.slice(0, len)
        if (w.length > len && w.endsWith(head)) {
          w = w.slice(0, w.length - len)
          changed = true
          break
        }
      }
...
/**
 * 一个引导语窗口对应的候选写法（抽取用，`guessNameFromWindow` 只给一个最优）。
 *
 * 为什么现在是「剥完修饰语的整体」**一个**：
 *   早期版本还会取窗口的末尾/开头 2~3 字当候选，结果在真机整本书抽取时抽出一大堆词尾碎片 ——
 *   `的时候说道` → 「时候」、`他站起来说道` → 「起来」、`已经点亮了三` → 「已经」。
 *   碎片能通过字型检查，又因为本身是高频词（几千次）而通过「出现次数」过滤，非常顽固。
 *   根因是**用定长切片去猜人名边界**；正确做法是把修饰语剥干净（`stripCueModifiers` 现在也能
 *   剥「半截修饰语」），剩下的整体就是候选；边界问题交给调用方的语料证据（片段收敛 / 简称并入）。
 *
 * 返回值仍保留数组形状（调用方按多候选处理），并且都过了 `looksLikePersonName`。
 */
export function guessNameVariants(window: string): string[] {
  const w = stripCueModifiers(window)
  if (w.length === 0) return []
  if (NON_NAME_TOKENS.includes(w)) return []
  // 剥剩下的东西本身可能就是一个修饰语（`猛然道，` → 窗口「猛然」）—— 那不是人名
  if (CUE_MODIFIERS.includes(w)) return []
  if (!looksLikePersonName(w)) return []
  return [w]
}
