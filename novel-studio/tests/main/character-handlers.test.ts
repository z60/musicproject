 ...
 *
 * ⚠️ 主角必须出现 **≥3 次**：抽取后要过一遍上游
 * `Online-novel-character-extraction` 的统计规则（出现次数 < 3 视为碎片删掉，
 * 见 `shared/canvas/character-profile.ts`）。真实小说里主角当然不止 3 次 ——
 * 这里把夹具写成那样，而不是为了迁就旧夹具去放宽规则。
 */
const CHAPTER_TEXT = [
  '萧炎沉声道：「药老，我来了。」',
  '药老捋着胡须笑了笑，没有回答。',
